<template>
      ...
      <CCard class="w-100">
        ...
        <div v-if="posts.comments">
          <CCardFooter class="w-100">
            <div class="w-100">
              <b>Comments</b>
              <span v-if="posts.comments.length === 0" style="opacity: 0.7;"> - There are no comments on this
                post.</span>
            </div>
            <div v-for="(comment, index) in posts.comments" :key="comment.id"
              class="mt-3 d-flex w-100 justify-content-between align-items-center">
              <div v-if="editingCommentId === comment.id" class="d-flex flex-grow-1">
                <CFormInput v-model="comment.comment_body" class="me-2 flex-grow-1"
              ...
              <div v-else class="d-flex justify-content-between align-items-center w-100"
                :style="{ borderBottom: index !== posts.comments.length - 1 ? '1px solid #dee2e6' : 'none' }">
  ...
  <CModal :visible="visibleUpdate" @close="() => { visibleUpdate = false }" aria-labelledby="LiveDemoExampleLabel">
    ...
    <CModalFooter>
      ...
      <CButton color="primary" @click="updatePost(posts.id)"
        :disabled="!updatepostdata.postTitle || !updatepostdata.postBody">Save changes</CButton>
    </CModalFooter>
  </CModal>
